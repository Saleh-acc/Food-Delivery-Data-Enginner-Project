...
def initialize_read_strem(spark:pyspark.sql.session.SparkSession):
    return (
        ...
        .load("lake.bronze.order_items")
    )

def transform(df_raw: pyspark.sql.DataFrame, 
              batch_id:int, 
              schema:pyspark.sql.types.StructType, 
              table_name: str = 'order_items') -> pyspark.sql.DataFrame:
    """
    ...
    df_order_items_flatern = flatten_kafka_payload(df = df_raw,
                                                          table_schema = schema ).dropDuplicates(['order_item_id', 'source_lsn'])
   
    max_lsn = df_order_items_flatern.agg({"source_lsn": "max"}).collect()[0][0] if "source_lsn" in df_order_items_flatern.columns else None
        
    print(f"[{table_name}] batch {batch_id}:"
                  f"max_source_lsn={max_lsn}")
    spark = df_order_items_flatern.sparkSession
    ...
    print(f"[{table_name}]  Is there data in the silver layer?",not df_order_items_ids.isEmpty())
    print(f"[{table_name}]  Is there data after left join the bronze layer with silver layer?",not df_order_items_joined.isEmpty())
...
def load_to_silver(df_transformed:pyspark.sql.DataFrame,
    ...
    df_transformed.createOrReplaceTempView("order_items_apending")    
...
if '__main__' == __name__:
    ...
    parser.add_argument("--checkpoints", required=True, default='s3a://lake/spark/checkpoints/silver_writer/order_items')
